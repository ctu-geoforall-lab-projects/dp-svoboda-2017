...
Grafické uživatelské rozhraní zásuvného modulu je reprezentováno jedním oknem třídy \texttt{QDockWidget}\footnote{\url{http://pyqt.sourceforge.net/Docs/PyQt4/qdockwidget.html}}, jehož hlavní výhodou je možnost ukotvení do~samotného programu QGIS. Díky tomu není nutné přepínat mezi~okny a~práce s~pluginem se stává uživatelsky přívětivou. 

Protože zásuvný modul se řídí podle~legislativy České republiky a~používá výmě\-nný formát katastru nemovitostí, je grafické uživatelské rozhraní v~českém jazyce. 
...
\begin{table}[H]
    ...
    \end{tabular}
    \centering
    \caption[VFK Driverem~– porovnání rychlosti načtení]{VFK Driverem~– porovnání rychlosti načtení}
...
Pokud v~adresáři, ve~kterém se nachází vstupní \zk{VFK} soubor, neexistuje databáze SQLite se~stejným názvem, vytvoří se pomocí \zk{VFK} Driveru knihovny GDAL (viz ukázka kódu~\ref{vytvoreni_db_vfk_driver}).
...
%% TODO - zmenit pismo tabulek
	\begin{figure}[H]
		...
		\caption[Načítání \zk{VFK} souboru~– diagram algoritmu]{Načítání \zk{VFK} souboru~– diagram algoritmu}
...
\begin{table}[H]
    \begin{tabular}{|l|l|}
        ...
        \hline
        \hline
          \texttt{\detokenize{KMENOVE_CISLO_PAR}} & \texttt{KMENOVE C. (PUV.)} \\ \hline
          \texttt{\detokenize{PU_PODDELENI_CISLA_PAR}} & \texttt{PODDELENI C. (EDI.)} \\ \hline
          \texttt{\detokenize{PODDELENI_CISLA_PAR}} & \texttt{PODDELENI C. (PUV.)} \\ \hline
          \texttt{\detokenize{PU_KMENOVE_CISLO_PAR}} & \texttt{KMENOVE C. (EDI.)} \\ \hline
          \texttt{\detokenize{PU_KATEGORIE}} & \texttt{KATEGORIE} \\ \hline
          \texttt{\detokenize{VYMERA_PARCELY}} & \texttt{VYMERA (SPI)} \\ \hline
          \texttt{\detokenize{PU_VYMERA_PARCELY}} & \texttt{VYMERA (SGI)} \\ \hline
          \begin{tabular}{@{}l@{}} \texttt{\detokenize{PU_VYMERA_PARCELY}} \\ \texttt{\detokenize{_ABS_ROZDIL}} \end{tabular} & \texttt{ROZ. VYMER} \\ \hline
          \begin{tabular}{@{}l@{}} \texttt{\detokenize{PU_VYMERA_PARCELY}} \\ \texttt{\detokenize{_MEZNI_ODCHYLKA}} \end{tabular} & \texttt{MEZ. ODCH. ROZ. VYMER} \\ \hline
          \texttt{\detokenize{PU_VZDALENOST}} & \texttt{VZDALENOST} \\ \hline
          \texttt{\detokenize{PU_CENA}} & \texttt{CELK. CENA} \\ \hline
          \begin{tabular}{@{}l@{}} \texttt{\detokenize{PU_BPEJ}} \\ \texttt{\detokenize{_BPEJCENA_VYMERA_CENA}} \end{tabular} & \begin{tabular}{@{}l@{}} \texttt{BPEJ KOD-CENA ZA M2} \\ \texttt{-VYMERA-CENA} \end{tabular} \\ \hline
...
Kvůli zařazení parcel do~kategorií se během načítání přidává do~vrstvy parcel sloupec \texttt{\detokenize{PU_KATEGORIE}} (alias \texttt{KATEGORIE}), jehož datový typ je celé číslo (\texttt{integer}). Zásuvný modul místo dlouhých názvů jednotlivých kategorií používá číslice \texttt{0} - \texttt{2}, viz~tab~\ref{tab:kategorie_hodnoty}. 
...
Kontrola \textit{obvodem} slouží k~výběru parcel, které se nenachází kompletně uvnitř vrstvy obvodu.
...
Jak vyplývá z~názvu, kontrola \textit{není v~SPI} provádí výběr parcel, které nejsou uvedeny v~souboru popisných informací. Pro výběr používá sloupec \texttt{\detokenize{KMENOVE_CISLO_PAR}}, neboť patří mezi povinně vyplněné \citep{struktura_vfk}. Pokud má parcela tento sloupec prázdný, znamená to, že se jedná o~chybu nebo nově vytvořenou parcelu.
...
Do~kontroly kromě vrstvy parcel vstupuje i~vrstva referenčního bodu, která musí obsahovat právě jeden prvek a~kvůli zamezení neočekávaných výsledků musí mít stejný souřadnicový systém jako vrstva parcel.
